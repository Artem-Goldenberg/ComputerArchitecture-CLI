...
int main() {
    ...
    if( pid == 0 ) { // child
        ...
        std::vector<std::string> input = {
            "FiLe8_NAME5=\"test2.txt\"",
            "cat $FiLe8_NAMES | grep 'std' "
        };
        std::string true_ans = "\n"
            "Welcome to myshell\n"
            "Shell> Shell> \n"
            "Failed to find the variable: FiLe8_NAMES\n\n"
            "Shell> \n\n"
            "Captured SIGINT(2) signal. Gracefully shutting down...\n\n";
        ...
        kill(pid, SIGINT);
        std::cout << "Kill signal sent" << std::endl;
        ...
        std::cout << "Wait status: " << status << '\n' << WIFEXITED(status) << '\n' << WEXITSTATUS(status) << '\n' << WIFSIGNALED(status) << '\n' << WTERMSIG(status) << std::endl;

        // return 1;

        if( WTERMSIG(status) != 2 ) {
            std::cout << "Wrong signal sent: " << WTERMSIG(status) << " , expexted 2 (SIGINT)" << std::endl;
        }
