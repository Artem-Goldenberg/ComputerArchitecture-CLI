...
int main() {
    // init test input
    Parser parser;
    std::vector<Token> in_tokens; // g++ lib.hpp -o lib.so | LD_PRELOAD=lol=
    ...
    in_tokens.push_back(Token(TokenType::mString, "lol"));
    in_tokens.push_back(Token(TokenType::mEquals, ""));

    // get parser results
    std::variant<std::vector<CommandData>, std::string> var_ans = parser.parse(in_tokens);
    if( std::holds_alternative<std::string>(var_ans) ) {
        if(std::get<std::string>(var_ans) == "equality error") {
            return 0;
        }
        std::cout << "Parse error: " << std::get<std::string>(var_ans) << ", but expected error: equality error" << std::endl;
        return 1;
    }
